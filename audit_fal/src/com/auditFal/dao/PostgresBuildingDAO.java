...
public class PostgresBuildingDAO extends BuildingDAO {
	...
	public Long create(Connection connection, Building building) throws DAOException {
		...
		try {
	        ...
	        if ( autoGeneratedKey.next() ) {
	        	...
	        	this.syncDpendencies(connection, itemId, building.getDependencies());
	...
	public Building find(Connection connection, Building building) throws DAOException {
		return this.findByLowerName(connection, building.getLowerName());
	}
	...
	public void update(Connection connection, Building building) throws DAOException {
		...
		try {
			...
			this.syncDpendencies(connection, id, building.getDependencies());
	...
	private void syncDpendencies(Connection connection, Long itemId, ArrayList<Long> dependencies) throws SQLException {
		...
		try {
			//get all junctions
			preparedStatement = DAOUtils.initPreparedStatement(connection, SQL_BUILDINGS_WORKSTATIONS_FIND, false, itemId);
			result = preparedStatement.executeQuery();
			...
			for(Long buildingId : junctionsToRemove) {
				DAOUtils.closeStatement(preparedStatement);
				preparedStatement = DAOUtils.initPreparedStatement(connection, SQL_BUILDINGS_WORKSTATIONS_DELETE, false, itemId, buildingId);
			...
			for(long id : dependencies) {
				if(!junctionsToKeep.contains(id) ) {
					preparedStatement = DAOUtils.initPreparedStatement(connection, SQL_BUILDINGS_WORKSTATIONS_CREATE, false, itemId, id);
					preparedStatement.executeUpdate();
				}
			}
